...
import matplotlib
from ..mortgage_utils.mortgage_plotter_util import plot_monthly_payments, \
...
class MortgageTrack(ABC):
    ...
    def __init__(self, interest_rate: float, num_payments: int, initial_loan_amount: int,
                 linked_index: Optional[List[float]] = None,
                 forecasting_interest_rate: Optional[List[float]] = None,
    ...
    def calculate_initial_monthly_payment(self) -> int:
        # Check for zero values and handle appropriately
        if self.num_payments == 0 or self.initial_loan_amount == 0:
            return 0
        # TODO: Check how its work with the interest only period
        return npf.pmt(self.interest_rate / MortgageConstants.MONTHS_IN_YEAR, self.num_payments,
    ...
    def plot_principal_and_interest_payments(self) -> None:
        ...
        :return: None
        """
        plot_principal_and_interest_payments(self.num_payments, self.get_principal_payments(),
