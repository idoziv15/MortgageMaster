...
from .mortgage_enums.interest_type import InterestType
from .mortgage_tracks.mortgage_track import MortgageTrack
from .mortgage_tracks.eligibility import Eligibility
from .mortgage_utils.mortgage_financial_utils_il import calculate_discount_factor
from .mortgage_enums.linkage_type import LinkageType
...
class MortgagePipeline:
    ...
    def calculate_weighted_average_interest_rates(self) -> float:
        ...
        """
        return sum(
            [track.interest_rate * percentage for track, percentage in self.get_tracks_percentages_dic().items()])
    ...
    def plot_interest_and_principal_payments(self) -> None:
        ...
        :return: None
        """
        plot_principal_and_interest_payments(self.get_num_of_payments(), self.get_principal_payments(),
                                             self.get_interest_payments())
    ...
    def calculate_normalize_resource_allocation(self) -> Dict[MortgageTrack, float]:
        ...
        """
        return {track: track.calculate_initial_monthly_payment() / self.calculate_initial_monthly_payment() for track in
                self.tracks}
    ...
    def calculate_index_linkage_segmentation(self) -> Dict[LinkageType, float]:
        ...
        """
        # TODO
        return {LinkageType.Linked: sum(
            [percentage for track, percentage in self.get_tracks_percentages_dic().items() if
             track in LinkageType.Linked.value]),
            LinkageType.NotLinked: sum(
                [percentage for track, percentage in self.get_tracks_percentages_dic().items() if
                 track in LinkageType.NotLinked.value]),
            LinkageType.Prime: sum([percentage for track, percentage in self.get_tracks_percentages_dic().items() if
                                    track in LinkageType.Prime.value])}
    ...
    def calculate_interest_type_segmentation(self) -> Dict[InterestType, float]:
        ...
        #             sum_i +=  percentage
        # TODO
        return {InterestType.Constant: sum(
            [percentage for track, percentage in self.get_tracks_percentages_dic().items() if
             any(isinstance(track, class_type) for class_type in InterestType.Constant.value)]),
            InterestType.NotConstant: sum(
                [percentage for track, percentage in self.get_tracks_percentages_dic().items() if
                 any(isinstance(track, class_type) for class_type in InterestType.NotConstant.value)]),
            InterestType.Prime: sum(
                [percentage for track, percentage in self.get_tracks_percentages_dic().items() if
                 track in any(isinstance(track, class_type) for class_type in InterestType.Prime.value)])}

    def calculate_total_cost_of_borrowing(self, years_to_exit: Optional[int] = None,
                                          average_interest_in_early_payment: Optional[
                                              Dict[MortgageTrack, float]] = None) -> int:
        """
        Calculate the total cost of borrowing over the investment period.

        :param years_to_exit: Optional. Number of years until exit. If not provided, calculates the total cost of
        :borrowing until the end of the loan term.
        :param average_interest_in_early_payment: Optional. Dictionary mapping MortgageTrack instances to their average
        :interest rates during early payments.
        ...
        if years_to_exit is None or average_interest_in_early_payment is None:
            ...
            return round(
                sum(self.get_interest_payments()[:months_to_exit]) + self.calculate_early_payment_fee(months_to_exit,
                                                                                                      average_interest_in_early_payment))
