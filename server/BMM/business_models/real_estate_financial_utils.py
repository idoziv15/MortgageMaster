...
def gross_yield(annual_rent_income: int, purchase_price: int) -> float:
    ...
    """
    return 0 if purchase_price == 0 else (annual_rent_income / purchase_price) * 100


def debt_service_coverage_ratio(net_operating_income: int, annual_loan_payment: Union[int, List[int]]) -> Union[
    float, List[float]]:
    """
    ...
    if isinstance(annual_loan_payment, list):
        return [0 if payment == 0 else net_operating_income / payment for payment in annual_loan_payment]
    return 0 if annual_loan_payment == 0 else net_operating_income / annual_loan_payment
...
def debt_to_income(total_debt_payments: Union[int, List[int]], annual_income: int) -> Union[float, List[float]]:
    ...
    if isinstance(total_debt_payments, list):
        return [0 if annual_income == 0 else (payment / annual_income) * 100 for payment in total_debt_payments]
    return 0 if annual_income == 0 else (total_debt_payments / annual_income) * 100
...
def loan_to_cost(loan_amount: int, total_project_cost: int) -> float:
    ...
    """
    return 0 if total_project_cost == 0 else (loan_amount / total_project_cost) * 100
...
def loan_to_value(loan_amount: int, after_repair_value: int) -> float:
    ...
    """
    return 0 if after_repair_value == 0 else (loan_amount / after_repair_value) * 100
...
def cash_of_cash(net_annual_cash_flow: int, total_equity_needed_for_purchase: int) -> float:
    ...
    """
    return 0 if total_equity_needed_for_purchase == 0 else round(
        (net_annual_cash_flow / total_equity_needed_for_purchase) * 100, 2)
...
def return_on_investment(net_profit: int, total_equity_needed_for_purchase: int) -> float:
    ...
    """
    return 0 if total_equity_needed_for_purchase == 0 else round((net_profit / total_equity_needed_for_purchase) * 100,
                                                                 2)
...
def cap_rate(net_operating_income: int, purchase_price: int) -> float:
    ...
    """
    return 0 if purchase_price == 0 else (net_operating_income / purchase_price) * 100
...
def multi_family_value_estimation(net_operating_income: int, cap_rate: int) -> int:
    ...
    """
    return 0 if cap_rate == 0 else round(net_operating_income / cap_rate)
